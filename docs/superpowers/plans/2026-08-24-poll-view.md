...
- [ ] **Step 6: Verify the quoted-reply preview**

Reply to the poll message from the other client with any text. Verify the reply shows a "Replying to ..." quote block reading `[Poll] <question>` (not the raw fallback text).

Correction from the final whole-branch review: the chat list row does not render a message preview at all — `Chat`/the chat-list composable only shows the chat name and unread count, so `messagePreviewText`'s `"poll"` case has no chat-list-row call site to exercise. Its only two call sites are the quoted-reply preview (exercised above) and the reaction-picker header (`MainActivity.kt:873`) — open the reaction picker on the poll message (same long-press/action used for reacting to any other message) and verify its header line also reads `[Poll] <question>`.
